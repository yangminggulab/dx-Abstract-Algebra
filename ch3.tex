...
\begin{note}[单位与相伴元辨析]
单位不是单位元：单位是有乘法逆元的元素，单位元是乘法中的 $1$。$b = \varepsilon a$（$\varepsilon$ 是单位）则 $b$ 是 $a$ 的相伴元。单位乘自己的逆元等于单位元（$\varepsilon\cdot\varepsilon^{-1}=1$）。在 $\mathbb{Z}$ 中单位是 $\pm1$；在 $\mathbb{Z}[i]$ 中单位是 $\pm1,\pm i$。
\end{note}

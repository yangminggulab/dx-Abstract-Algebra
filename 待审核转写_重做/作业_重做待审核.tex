...
\textbf{解：} 设 $R$ 是满足条件的有限环，$a\in R$，$a\ne0$。考虑映射 $x\mapsto ax$（$x\in R$）。由于 $R$ 没有零因子，若 $ax=ay$ 则 $a(x-y)=0$ 从而 $x=y$，故映射是单射。$R$ 有限，故映射也是满射。特别地，存在 $b\in R$ 使 $ab=a$，从而 $ab-a=0$，即 $a(b-1)=0$，因为 $a\ne0$ 且无零因子，所以 $b=1$，即 $R$ 有乘法单位元。又存在 $c\in R$ 使 $ac=1$，故每个非零元有右逆元，类似地有左逆元，因此 $R$ 是除环。
\end{example}

...
\begin{note}[变换群的意义]
在群论中，"代表元"是指从每个陪集中选取的一个元素，用来"代表"整个陪集。例如，如果 $G$ 是一个群，$H$ 是 $G$ 的一个子群，那么对于任一 $g\in G$，左陪集定义为 $gH=\{gh:h\in H\}$。在这个陪集中，我们可以任取一个元素（通常就是 $g$ 本身）作为该陪集的"代表元"。这个代表元并不是唯一的，但它能帮助我们在讨论商群 $G/H$ 时用符号 $gH$ 来表示整个陪集。简单来说，代表元就是每个陪集中用来标记该陪集的一个选定的元素。
\end{note}
